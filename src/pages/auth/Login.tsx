...
export function Login() {
    ...
    return (
        <div className={cn('w-full h-screen flex flex-col bg-stone-100 px-4')}>
                <div className={cn('mt-8')}>
                    <div className={cn('container mx-auto')}>
                            <h1 className={cn("text-2xl font-Poppins_Bold")}>LegalEase</h1>
                ...
                <div className={cn('w-full mx-auto flex flex-col flex-1 gap-8 justify-center items-center')}>
                    <div className={cn('p-6 rounded-lg w-full container mx-auto sm:w-96 lg:shadow lg:bg-white')}>
                        <div className={cn('flex items-left')}>
                            <h1 className={cn("text-3xl text-left font-Poppins_Semibold mb-6")}>Sign In</h1>
                        </div>
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className={cn('space-y-4')}>
                                ...
                                <InputField
                                    ...
                                    isRequired={true}
                                />
                                <div className={cn('flex justify-end items-end my-4')}>
                                    <Link to={""} className={cn("text-blue-500 text-sm underline")}>Forgot Password?</Link>
                                </div>
                                <Button type="submit" className={cn("w-full font-Poppins_Semibold")}>Sign In</Button>
                            </form>
                        </Form>
                    </div>

                </div>
                <div className={cn('gap-4 container mx-auto h-20 justify-center items-center hidden lg:flex')}>
                    <Link to={""} className={cn("text-sm font-Poppins_Semibold ")}>LegalEase</Link>
                    <Link to={""} className={cn("text-sm ")}>User Agreements</Link>
                    <Link to={""} className={cn("text-sm")}>Privacy Policy</Link>
                    <Link to={""} className={cn("text-sm")}>Community Guidelines</Link>
                    <Link to={""} className={cn("text-sm")}>Copylight Policy</Link>
                    <Link to={""} className={cn("text-sm")}>Send Feedback</Link>
